...
######
cat("\nThis temp code is for the group to file name matching issue \n")
cat("\nAnd would replace that code later \n")
...
if(grepl("-h",args[6])) {
  ...
  cat("\n####################################################\n")
  cat("\nVersion: 1.1a (Last modified on 06-28-2018)\n")
  cat("\nUsage: Rscript RNA-seq_DEG_with_DE_in_GO.R EXPRESSION_TABLE DESIGN_TABLE CPM_CUTOFF PVAL_CUTOFF L2F_CUTOFF PERFORMGO_LOGIC SPECIES GO_PVAL_CUTOFF\n")
  ...
  inList=rownames(colData)[colData$condition==levels(colData$condition)[k0]|colData$condition==levels(colData$condition)[k1]]
  ...
  cpm=cpm[rowSums(cpm[,inList]>as.numeric(args[8]))>=min(table(colData[,1])[c(k0,k1)]),]
  ...
  cat("\nWriting results ...\n")
  cpm=cpm[,rownames(colData)[colData$condition%in%levels(colData$condition)[c(k0,k1)]]]
  ...
  write.csv(res,paste0(paste(levels(colData$condition)[c(k0,k1)],collapse="_vs_"),"_DEG_full_list.csv"))
  
  res2=res[res$padj<as.numeric(args[9]) & abs(res$log2FoldChange)>as.numeric(args[10]),]
  write.csv(res2,paste0(paste(levels(colData$condition)[c(k0,k1)],collapse="_vs_"),"_DEG_significant_only.csv"))
  ...
  print(ggplot(pcaData,aes(x=PC1,y=PC2,color=condition))+
          ...
          geom_text(aes(label=name,fontface=2),hjust="inward",vjust=-0.8,size=2)+
          xlab(paste0("PC1: ",percentVar[1],"% variance"))+
          ylab(paste0("PC2: ",percentVar[2],"% variance"))+
          theme(plot.title=element_text(size=14,family="Helvetica",face="bold",hjust=0.5),
                text=element_text(size=12,family="Helvetica"),
  ...
  if(args[11]=="TRUE" & dim(res2)[1]>0) {
    ...
    res2.up=res2[res2$log2FoldChange>=0,]
    res2.down=res2[res2$log2FoldChange<0,]
    #####################
    # up-regulated DEGs #
    #####################
    # extract Entrez IDs
    suppressMessages(E_IDs<-select(get(xegdb),rownames(res2.up),c("ENTREZID","GENENAME"),"ALIAS"))
    ...
    if(sum(Entrez%in%universe)>0) {
      ...
      write.csv(resultBP,file=paste0("GO_BP_up-regulated_in_",levels(colData$condition)[k0],".csv"))
      htmlReport(hgOverBP,file=paste0("GO_BP_up-regulated_in_",levels(colData$condition)[k0],".html"),digits=5)
      ...
      write.csv(resultCC,file=paste0("GO_CC_up-regulated_in_",levels(colData$condition)[k0],".csv"))
      htmlReport(hgOverCC,file=paste0("GO_CC_up-regulated_in_",levels(colData$condition)[k0],".html"),digits=5)
      ...
      write.csv(resultMF,file=paste0("GO_MF_up-regulated_in_",levels(colData$condition)[k0],".csv"))
      htmlReport(hgOverMF,file=paste0("GO_MF_up-regulated_in_",levels(colData$condition)[k0],".html"),digits=5)
      ...
      write.csv(resultKEGG,file=paste0("KEGG_up-regulated_in_",levels(colData$condition)[k0],".csv"))
    } else {
    ...
    # down-regulated DEGs #
    #######################
    suppressMessages(E_IDs<-select(get(xegdb),rownames(res2.down),c("ENTREZID","GENENAME"),"ALIAS"))
    Entrez=unique(E_IDs[!is.na(E_IDs$ENTREZID),2])
    universe=mappedkeys(get(xegGO))
    ...
    if(sum(Entrez%in%universe)>0) {
      ...
      write.csv(resultBP,file=paste0("GO_BP_up-regulated_in_",levels(colData$condition)[k1],".csv"))
      htmlReport(hgOverBP,file=paste0("GO_BP_up-regulated_in_",levels(colData$condition)[k1],".html"),digits=5)
      ...
      write.csv(resultCC,file=paste0("GO_CC_up-regulated_in_",levels(colData$condition)[k1],".csv"))
      htmlReport(hgOverCC,file=paste0("GO_CC_up-regulated_in_",levels(colData$condition)[k1],".html"),digits=5)
      ...
      write.csv(resultMF,file=paste0("GO_MF_up-regulated_in_",levels(colData$condition)[k1],".csv"))
      htmlReport(hgOverMF,file=paste0("GO_MF_up-regulated_in_",levels(colData$condition)[k1],".html"),digits=5)
      ...
      write.csv(resultKEGG,file=paste0("KEGG_up-regulated_in_",levels(colData$condition)[k1],".csv"))
    } else {
      cat("\nGene IDs of DEGs cannot be identified in the GOgeneIds!!!\nIs the SPECIES correctly specified?")
    }
