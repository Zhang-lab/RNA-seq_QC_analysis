...
s0_rna_pre () {
    ...
    ln -s `realpath $R1`    ./'Processed_'$name/
    ln -s `realpath $R2 2> /dev/null`  ./'Processed_'$name/  2> /dev/null
    cd ./'Processed_'$name/
...
cal_score () {
        ...
        _uniq=`awk 'BEGIN {OFS=FS="\t"}NR==1{for(i=1;i<=NF;i++){if($i=="uniq_ratio"){getline; print $i}}}' QC_summary*.txt` \
            && if (( $(echo "$_uniq >= 0.65" | bc -l) )); then _status=1; else _status=0; fi \
