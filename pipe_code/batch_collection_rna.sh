...
#data collection
for file in `find .. -name "RNA_QC_data_collection_*"`   
do
    name=`echo ${file#*RNA_QC_data_collection_}`
    #1, gene type distribution
    cd $file
    cp QC_summary*.txt  $temp
...
head -1 `ls QC_summary_* | head -1` > merged_RNA_qc_table.txt
for file2 in `ls QC_summary_*`
do
