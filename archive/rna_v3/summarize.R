...
part3=data.frame("STAR","2.5.4b","--outWigType bedGraph --outWigNorm RPM --outSAMtype BAM SortedByCoordinate --outWigStrand Stranded --quantMode TranscriptomeSAM",as.numeric(qc[1,4]),as.numeric(qc[1,7]),as.numeric(qc[1,8]),as.numeric(qc[1,10]),splice)
colnames(part3)=c("alignment_program","alignment_program_version","alignment_program_parameters","average_reads_length","reads_aligned_0_time","reads_aligned_exactly_1_time","reads_aligned_greater_than_1_time","number_of_splice")
...
part4=data.frame("featureCounts","1.5.1","gencode.v27.annotation.gtf","-p -g <gene_name>/<gene_type> -Q 10 -s <0,1,2>",as.numeric(round(qc[1,12],4)),as.numeric(round(qc[1,13],4)),as.numeric(qc[1,11]),as.numeric(qc[1,14]),as.numeric(qc[1,15]),ex.dis,gene_type)
colnames(part4)=c("software","version","annotation_file_version","count_parameter","number_of_uniquely_mapped_fragments_with_gene_feature","ratio_of_uniquely_mapped_fragments_with_gene_feature","area_under_curve_of_gene_body_coverage","detected_genes_number","detected_genes_with_cpm_larger_than_1","detected_genes_cpm_distribution","gene_type_fragment_count")
