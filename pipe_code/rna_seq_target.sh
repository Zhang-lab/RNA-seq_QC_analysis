...
# Preparation:
date
pipe_version='target_190311'
host="zhanglab/rna-seq base"
...
do 
    ...
    h) echo "usage:  path-to-pipe/pipe.sh  -g <hg38/hg19/mm10/danRer10>  -r <PE/SE> -o read_file1  -p read_file2(if necessary)" && exit;;
    [?]) echo "usage:  path-to-pipe/pipe.sh  -g <hg38/hg19/mm10/danRer10>  -r <PE/SE> -o read_file1  -p read_file2(if necessary)";;
    esac
done
...
s4.1_fc () {
    # determine the strand by assigned reads (the highest one should be correct situation)
    # subsample chr10 from bam file
    samtools view -q 10 $bam_file chr10 -b > test_strand.bam

    # run 3 strand
    if [[ $types == PE ]];
        then
             $featureCounts -a $annotation_file -p -T 4 -Q 10 -s 0 -o test_s0_fc -g gene_name test_strand.bam
             $featureCounts -a $annotation_file -p -T 4 -Q 10 -s 1 -o test_s1_fc -g gene_name test_strand.bam
             $featureCounts -a $annotation_file -p -T 4 -Q 10 -s 2 -o test_s2_fc -g gene_name test_strand.bam
        elif [[ $types == SE ]];
        then
            $featureCounts -a $annotation_file -T 4 -Q 10 -s 0 -o test_s0_fc -g gene_name test_strand.bam  
            $featureCounts -a $annotation_file -T 4 -Q 10 -s 1 -o test_s1_fc -g gene_name test_strand.bam
            $featureCounts -a $annotation_file -T 4 -Q 10 -s 2 -o test_s2_fc -g gene_name test_strand.bam
    fi

    # check assigned reads
    s0_hit=`grep Assigned test_s0_fc.summary | awk '{print $2}'`
    ...
    # check strand infor and correct the bedgraph output
    r12=`python -c "print(1.0*$s1_hit/$s2_hit)"`
    r21=`python -c "print(1.0*$s2_hit/$s1_hit)"`
    if (( $(echo "$r12 >= 2" | bc -l) )); then
        strand=1
        ### fc strand seems opposite when showing on browser to the STAR, so we need to reverse strand1, this WON'T influence any other results but the visualization only
        rm step2.1_Star_${name}.Unique.stranded.bg.gz.tbi
        gunzip step2.1_Star_${name}.Unique.stranded.bg.gz
        awk '{if ($4!=0) {$4=$4*-1} print $1"\t"$2"\t"$3"\t"$4}' step2.1_Star_${name}.Unique.stranded.bg > temp \
            && mv temp step2.1_Star_${name}.Unique.stranded.bg \
            && bgzip step2.1_Star_${name}.Unique.stranded.bg \
            && tabix -p bed step2.1_Star_${name}.Unique.stranded.bg.gz
    elif (( $(echo "$r21 >= 2" | bc -l) )); then
        strand=2
    else
        strand=0
        ### change all bg signal to positive
        rm step2.1_Star_${name}.Unique.stranded.bg.gz.tbi
        gunzip step2.1_Star_${name}.Unique.stranded.bg.gz
        awk '{if ($4<0) {$4=$4*-1} print $1"\t"$2"\t"$3"\t"$4}' step2.1_Star_${name}.Unique.stranded.bg > temp \
            && mv temp step2.1_Star_${name}.Unique.stranded.bg \
...
s4.2_collect () {
    ...
    if [ $species == "mm10" ]; then
        grep Hbb 'step4.2_gene_name_count_'$name'.txt' | cut -f 1,3 > 'step4.2_temp_globin_'$name'.txt'
        grep Hba 'step4.2_gene_name_count_'$name'.txt' | cut -f 1,3 >> 'step4.2_temp_globin_'$name'.txt'
    fi
    ...
    grep -f <(echo -e "^rRNA\n^Mt\n^ribo") 'step4.2_gene_type_count_'$name'.txt' | cut -f 1,3 > step4.2_temp_store_$name'.txt'
...
cal_score () {
        echo -e "item\tnumber\tscore" > step4.6_score_calculation_${name}.result

        _uniq=`awk 'BEGIN {OFS=FS="\t"}NR==1{for(i=1;i<=NF;i++){if($i=="uniq_ratio"){getline; print $i}}}' QC_summary*.txt` \
            && if (( $(echo "$_uniq >= 0.35" | bc -l) )); then _status=1; else _status=0; fi \
            && echo -e "uniq_ratio\t$_uniq\t$_status" >> step4.6_score_calculation_${name}.result \
            && unset _status

        _assign=`awk 'BEGIN {OFS=FS="\t"}NR==1{for(i=1;i<=NF;i++){if($i=="assigned_reads_number"){getline; print $i}}}' QC_summary*.txt` \
            && if (( $_assign >= 10000000 )); then _status=1; else _status=0; fi \
            && echo -e "assign_reads\t$_assign\t$_status" >> step4.6_score_calculation_${name}.result \
            && unset _status

        _cpm=`awk 'BEGIN {OFS=FS="\t"}NR==1{for(i=1;i<=NF;i++){if($i=="genes_CPM_gt_1"){getline; print $i}}}' QC_summary*.txt` \
            && if (( $_cpm >= 9000 )); then _status=1; else _status=0; fi \
            && echo -e "genes_w_CPM_gt1\t$_cpm\t$_status" >> step4.6_score_calculation_${name}.result \
            && unset _status
}
    ...
    sed 's/\[/{/g' $name'_report.json' | sed '/    {/d' | sed '/\]/d' | sed 's/    }/  },/g' | sed 's/"!/{/g' | sed 's/!"/}/g' | sed 's/"?/[/g' | sed 's/?"/]/g' | sed 's/@/"/g' | tac | sed '2s/},/}/g' | tac | sed 's/\\r//g' | sed "s/MD5ToBeChange/$md5/g" > QC_$name'.json'
    name_gtf=`echo ${annotation_file##*/}`
    sed -i "s/gtf_ToBeChange/$name_gtf/g" QC_$name'.json'
